...
import { readRawExcel, parseExcelFile } from "@/lib/excel-parser";
import { useNexusStore } from "@/lib/store";
import { ColumnMappingModal } from "@/components/column-mapping-modal";
import { CATALOGUE_REQUIRED } from "@/lib/column-mapping";
...
export function UploadZone() {
  ...
  const handleConfirmMapping = useCallback(
    (mapping: ColumnMapping) => {
      ...
      try {
        const products = parseExcelFile(pendingData.buffer, mapping);
        setCatalog(products);
  ...
  return (
    <>
      ...
      <ColumnMappingModal
        ...
        rawData={pendingData}
        mode="catalogue"
        requiredFields={CATALOGUE_REQUIRED}
