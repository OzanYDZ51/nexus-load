...
import { parseOrderFile } from "@/lib/excel-parser";
    ...
    (file: File) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const result = parseOrderFile(buffer, catalog);

          if (result.items.length === 0) {
            toast.error(
              "Aucune référence trouvée. Vérifiez votre fichier."
            );
            return;
          }
  ...
  return (
    ...
    >
      ...
      <div className="text-text-secondary text-[13px] mb-3">
        {catalog.length > 0 ? (
          <>Glissez un fichier ou cliquez — colonnes : <span className="text-primary-cyan font-semibold">Référence</span>, <span className="text-primary-cyan font-semibold">Quantité</span></>
        ) : (
          <>Glissez un fichier ou cliquez — colonnes : <span className="text-primary-cyan font-semibold">Référence</span>, <span className="text-primary-cyan font-semibold">Quantité</span>, <span className="text-primary-cyan font-semibold">Poids</span>, <span className="text-primary-cyan font-semibold">Longueur</span>, <span className="text-primary-cyan font-semibold">Largeur</span>, <span className="text-primary-cyan font-semibold">Hauteur</span></>
        )}
      </div>
